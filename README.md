# 🚗 Car Rental Management System

## 📌 About the Project

A console-based **Java Car Rental Management System** developed to demonstrate Object-Oriented Programming (OOP) concepts, software design principles and AI-assisted vs traditional programming approaches. This project explores how software solutions can be developed using different programming approaches by comparing implementation structure, development efficiency and learning outcomes through practical development and documentation.

---

## 📌 Project Overview

The **Car Rental Management System** is a Java console application designed to manage the core operations of a vehicle rental service. The system allows users to manage vehicles, customers and rental transactions through a menu-driven interface. All data is stored temporarily using the **Java Collections Framework** without database integration.

The project focuses on applying fundamental software engineering concepts including:

- Object-Oriented Design
- Class Relationships
- Data Management
- Input Validation
- Application Workflow Design

---

## ✨ Features

### 🚘 Vehicle Management
- Register new vehicles
- View all registered vehicles
- Search vehicles by vehicle ID
- Remove vehicles from the system

### 👤 Customer Management
- Register new customers
- View customer information
- Search customers
- Remove customer records

### 📝 Rental Management
- Rent vehicles to customers
- Return rented vehicles
- Track rental information

---

## 🧩 Object-Oriented Programming Concepts Applied

This project demonstrates the following OOP principles:

### Classes and Objects
- Created reusable classes to represent real-world entities such as vehicles, customers and rentals.

### Encapsulation
- Protected class attributes using private access modifiers and controlled access through methods.

### Inheritance
- Implemented inheritance relationships between vehicle types.

Example:
```
Vehicle
 ├── Car
 ├── Van
 └── Motorcycle
```

### Abstraction
- Used abstract classes and methods to define common behaviours.

---

## 📂 Project Structure

```
Car-Rental-Management-System
│
├── 01. Manual Implementation (Version A)
├── 02. AI Assisted Implementation (Version B)
```

---

## ▶️ How to Run the Project

### Prerequisites

- Java Development Kit (JDK 21 or above)
- Any Java IDE (IntelliJ IDEA, Eclipse, NetBeans and etc.)

### Steps

1. Clone the repository:

```bash
git clone https://github.com/Nipun-Lakshan/car-rental-management-system.git
```

2. Open the project using a Java IDE.

3. Compile and run:

```
java Application.java
```

4. Use the console menu to interact with the system.

---

## 🎯 Project Objectives

The main objectives of this project are:

- Understand and apply Object-Oriented Programming concepts.
- Develop a real-world software solution using Java.
- Practice software design and implementation techniques.
- Compare AI-assisted programming with traditional software development approaches.
- Improve problem-solving and programming skills.

---

## 👨‍💻 Author

**Nipun Lakshan**

* Information Technology & Management Undergraduate  
* University of Colombo, Faculty of Science

GitHub:
https://github.com/Nipun-Lakshan

---

## ⭐ Support

If you find this repository useful, consider giving it a star.

---
